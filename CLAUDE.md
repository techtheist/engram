...
## Status (2026-07-10)
- **Phase 0 AND Phase 1 complete & verified** (107 tests: core 72, http 18, mcp 13, cli 4). `engram serve` serves the HTTP API + SSE + the embedded Vue pane at `http://127.0.0.1:8787`; release binary at `~/.cargo/bin/engram`. Latest published release: **v0.2.2** (2026-07-06). **v0.3.0 prepared** (user updates screenshots, then: push → `gh workflow run draft-release.yml -f version=0.3.0` → publish the draft in the GitHub UI, which tags and builds artifacts). All crates share `[workspace.package] version` (0.3.0) which release.yml stamps from the tag — never version a crate individually; `claude-plugin/.claude-plugin/plugin.json` must carry the same version (a cli test enforces it — the plugin installs from the repo, not from release artifacts).
- **Shipped 2026-07-10 (uncommitted — goes into v0.3.0):** **Claude Code plugin** (repo-root `.claude-plugin/marketplace.json` + `claude-plugin/` with relaxed-skill/hook copies, `/engram:setup` + `/engram:pane` commands, **no global MCP config** — deliberate, see PLAN §10; per-repo wiring via new `engram setup --mcp-only`; verbatim-copy sync tests in `crates/engram-cli/src/setup.rs`; the hook defers when `ENGRAM_HOOK_SOURCE=plugin` and the repo registers its own — never brief twice); pane CRUD + tags end to end; audit journal (per-mutation rows with binary context, pane Audit log drawer, `audit` MCP tool; node mutations attribute the *acting* session, creator only on "created" rows); session-start brief hook (`hooks/session-brief.sh` + `engram setup`); verified code refs / drift (engine scan + `GET /drift` + `list_drift` MCP tool + Review-panel section + drifted badges + health-strip count); `timeline` (MCP tool + `GET /nodes/{id}/timeline`, replaces-chain oldest-first, topology-ordered); `engram://node/{id}` MCP resource (list = 25 newest, template for the rest); brief format v2 (16k budget, node id on every record, Recently-added right after suspects, Decisions capped 7 with 80-char excerpts); SidePanel drawer shell (one drawer per side, drag-resizable session widths min…50vw, macOS overscroll contained inside the frame); four canvas layouts (Skyline default / Nebula / Archipelago / Orbit — see the graph's layout Decision for the physics); skill overhaul (value story, maintenance duties incl. close-the-loop + drift repair, example flows, unhappy-path protocol — all variants + agents/*.md).
...
- **Next (PLAN §10 near-term):** timeline pane view; `engram doctor`; bundle skill + `.mcp.json` bootstrap into the IDE plugins; `runIde`/`verifyPlugin`; MCP transport migration; session quarantine (journal substrate shipped); file-read match hook (design TBD — needs a code_refs-match endpoint + noise control). Later: Cursor/Windsurf MCP wiring; app-level DB encryption (far down). **Out of scope permanently:** multi-user & repo sync — a future enterprise product, not Engram Alpha (user decision 2026-07-10).
