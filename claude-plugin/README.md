...
| `hooks/session-brief.sh` | SessionStart hook — injects the graph's brief so every session starts already briefed. Silent in repos without an `.engram/` graph, and defers to a repo-level registration (`engram setup`) so the brief never injects twice. |
...
The plugin deliberately ships **no global MCP server**: `engram mcp` binds a `.engram/graph.db` in whatever project it starts in, and a plugin-level server would create one in every repo you open. MCP registration stays per-repo via `/engram:setup`.

## Capture intensity

The bundled skill is the **relaxed** variant. For a fuller graph, install `normal` or `aggressive` at project level — `engram setup --cli claude --skill aggressive` — a project skill overrides the plugin's. The three variants are documented in [`skills/engram/`](../skills/engram/) at the repo root.
