...
| Logic | do these two claims agree, disagree, duplicate? | runtime `Xenova/mobilebert-uncased-mnli` (quantized ONNX, 27 MB) since 0.7.2 — replaced `nli-deberta-v3-small` (172 MB) on the end-to-end contradiction benchmark, which showed the old model calling 80-86% of UNRELATED claims contradictions (five seeds); `finecat-nli-m` (100M, ModernBERT-base distillation) stays an eval-side candidate (no ONNX export) | **shipped v0.5.0** — `nli.rs` (FastNli), suspects carry NLI hints, `check_claim` buckets supports/contradicts/silent; eval harness `scripts/nli-eval.py` |
...
## 7D. v0.7.0 — the customization release (scoped 2026-07-22; RELEASED 2026-07-23)

*Status:* **shipped and published 2026-07-23** — feat commit 5b451f7 + docs d785481 on main, tag v0.7.0, all artifact jobs green, darwin checksum verified. The workstream notes below say "uncommitted draft" where they were written mid-build; all of it is in the release. The one scoped item that did NOT ship is the timeline-cards fifth layout (still blocked on user design notes — carried to §7E).

The release theme: **everything the engine hardcodes about meaning becomes per-graph configuration** — the ontology (types + verbs), the policy constants (decay/trust/thresholds), and the brief composition — stored in the graph itself, edited in one large Settings menu and over HTTP. Rides tepin-as-default: the by-default migration is what puts every graph on the store that carries its own config.

**Workstreams (build order):**

1. **Auto-migrate + storage default (roadmap 0.7.\*):** any open of a `graph.db` with a real embedder silently migrates it to a sibling `graph.tepin` (graph.db untouched as the backup, one-line notice); SQLite driver stays as a read/fallback path until 0.8 drops it. `export`/`import` become thin clients of the owning core (resolve repo daemon.json → machine core → direct-open fallback), which also makes import's SSE/conflict side effects live.
2. **Config substrate:** one per-graph config document in store meta (travels with migrate/export/import) with three sections — `ontology` (types: name, hue, durability default, role flags, brief/worklist/highlight behavior; verbs: name, sentence template, role flags), `policy` (decay windows, trust anchors, suspect/dupe/NLI thresholds, type priors), `brief` (total budget, per-section show/count/per-item cut: tags, recent, suspects, open work, per-type sections, home reserve, optional **ontology section**). `GET/PUT /projects/{sel}/config` (+ presets list/apply), validated against **hard invariants**: sentence-shaped edges only; exactly one supersession verb and one contradiction verb; trust/durability semantics attach to **role flags, never names**. Config-changed SSE event.
3. **Engine on live config** *(done 2026-07-23, uncommitted draft — live-verified: PUT /config hot-reloads the very next brief, stock brief byte-identical across the upgrade)*: `NodeType`/`EdgeType` stopped being closed enums — string-backed names (`Cow<'static,str>` newtypes; shipped names remain as consts) validated by SHAPE at parse and by EXISTENCE at the engine's write boundary against the graph's ontology (add/update node+edge, search type filters → `Error::Config` 400 naming the declared set). Every behavior keys on role flags: worklist (list_open, open-status default, brief heading names the worklist types), anchor (conflict-scan exclusion, sweep skips), supersession (timeline chain, replaces-verdict edge speaks the graph's verb), contradiction (demotion reconcile, conflict lists/taps/alerts, conflict-verdict edge), rank_prior (trust_boost), per-type durability defaults (MCP). The 14 policy tunables run live: trust anchors/floors/windows/stale parameterize `policy::trust`/`stale_since` (constants remain only as `Default` sources; degenerate tuning guarded — floor ≥ stale never decays), stores cache the parsed config (`Store::config()`, refreshed by `set_graph_config` — so import/migrate refresh too) and hydrate every row's trust through it; dupe/warn/suspect/NLI-gate/decay-TTL read config at the engine. Brief fully on `BriefConfig` (section toggles/caps/excerpts, ontology-teaching section, worklist heading, per-type canon sections iterate the ontology; brief-budget defaults resolve from the graph's config in HTTP/MCP/CLI, decay TTL likewise). `describe_ontology` MCP tool shipped early (renders `GraphConfig::describe_ontology`, same content as the brief section). Deliberately name-keyed still: home-graph promotion (Principle/Caution — the tracked role-mapping risk) and digest's FIXME→Problem/TODO→Intent suggestions. Tests 195 incl. a custom-ontology battery (fully renamed types/verbs run every role on both backends; tuned windows move trust/staleness; tuned dupe bar flips write verdicts; brief follows composition + speaks custom ontologies).
4. **Presets + Settings UI** *(landed 2026-07-23, uncommitted draft — HTTP flow verified on a scratch daemon; UI in user testing)*: **Backend:** 3 shipped presets (`engram` = the default set, `research` — Claim/Method/Question/Finding/Source/Task with `refutes` as the contradiction, `minimal` — Rule/Note/Todo) served by `GET /config/presets` (applying = plain PUT); `POST /config/rename-type|rename-verb` = **rename + bulk retype in one gesture** (new `Store::retype_nodes/retype_edges`, SQL UPDATE on sqlite; retype-then-persist ordering so a crash is re-runnable; roles/hue/brief ride the rename; audit `type_renamed`/`verb_renamed`); `set_graph_config` gained the **in-use guard** — a PUT dropping a type with stored nodes (or verb with edges) is a 400 pointing at the rename gesture, so config edits can never strand knowledge. **Pane went fully ontology-driven:** `NodeType`/`EdgeType` widened to strings; `constants/ontology.ts`'s hardcoded maps deleted; new `stores/config.ts` holds the fetched GraphConfig, derives node colors from each type's HUE (injected `--nt-*` CSS vars, light scheme via `[data-theme$="light"]`; `highlight:false` types render desaturated — role drives the muted anchor look) and edge colors/dash/animation **from verb roles** (contradiction red + animated, supersession amber, reason violet, answer green, dependency sky); all ~10 consumers (canvas, node cards, create/detail/connect/filter/search/review/checkup/health-strip) read the store, worklist/reason/supersession literals became role lookups; `trust.ts` explanations parameterized on the live policy; SSE `config_changed` → refetch config + graph; project switch reloads config. **New Settings → Graph settings drawer** (`ConfigPanel.vue` + `useGraphSettings`): preset shelf, per-type cards (rename→bulk-retype with node counts, hue slider + live swatch, thought, durability, worklist/anchor/highlight + rank prior, per-type brief section), per-verb cards (rename, reads_as, exactly-one supersession/contradiction as radio groups, reason/answer/dependency), all 14 policy knobs **with plain-word explanations rendered from the live values**, brief composition incl. the teach-ontology toggle; draft/dirty/save-revert, backend 400s surfaced verbatim. Deliberately deferred: empty-graph template proposal (cold-start touch), per-project persisted custom colors beyond hue (hue IS the persistence).
5. **Teaching surface** *(done 2026-07-23, uncommitted draft)*: optional brief ontology section + `describe_ontology` MCP tool (landed with workstream 3); digest + capture skill variants teach "the graph defines its ontology — call describe_ontology when names surprise you". **Generated-skill-files-from-ontology pulled forward from ~0.7.1** (user request): CLI `skillgen` — a graph on the shipped ontology installs the canonical variant text verbatim, a customized one gets a skill GENERATED in its own vocabulary (types with thoughts + worklist/anchor duties, verbs with reads_as + role duties, durability defaults, policy numbers in plain words, verdict protocol speaking the graph's supersession/contradiction verbs, a describe_ontology fallback note); `SkillAdmin` trait (engram-http, CLI-implemented like ModelAdmin) serves `POST /skills/install {variant}` per project (engine.repo_root; home graph 400), with setup's **symlink guard** (a symlinked skill dir — this repo's dogfood — is reported as a deliberate-sourcing no-op naming the link target, never written through and never an error; verified live on both paths). Pane: "Assistant skill" section in Graph settings — variant picker + Install-into-project. **No MCP config-write tools** — config stays a user gesture. *(Same session UI polish: project switching force-closes every open drawer via `usePanels.closeAll()` — drawers hold per-graph state and carrying them across graphs showed the previous project's truth as the new one's; native checkboxes/radios replaced app-wide with token-styled controls in base.css — `--check-accent` lets the redactor color each type card's checks with the type's own derived hue.)*
6. **Extras:** **file-read match hook DONE** *(2026-07-23, uncommitted draft — live-verified on the dogfood repo)*: `Engine::match_code_refs` (a ref matches the file exactly or any directory above it; active + non-stale only, trust-ordered) behind `GET /refs/match?path&session` which owns ALL noise control server-side — per-session dedupe (bounded seen-map in AppState; repeat reads surface the next tier instead of repeating), cap 3/injection, stale filter — and returns ready-to-inject markdown; `hooks/file-read-match.sh` (PostToolUse on Read|Edit|Write|MultiEdit, python3-powered JSON in/out, repo-daemon-first then machine-core via POST /projects, silent on every failure path) wired by setup (`.claude/hooks/engram-refs.sh` + registration), shipped in the plugin (hooks.json PostToolUse + verbatim copy, sync-tested), and dogfooded in this repo's settings.json. Ride-along: the brief hook's `graph.db`-only gate fixed for tepin-born repos. The "successful downstream use" confirmed_at carrier stays a follow-up (needs session-outcome semantics). **Same day, two NLI quality sweeps** (the coherence niche where MNLI's co-reference presupposition holds): `audit_coherence` (body→title entailment — "Check note quality" in Checkup; low entailment = the title overclaims) and `audit_stale_triage` ("Triage stale notes": live canon entails a stale note → reconfirm nomination, contradicts within the suspect band → judge as conflict, nothing speaks to it → archive candidate). **Activity journal**: `audit_activity` rows (`entity: session`) — `mcp_session_started`/`mcp_session_ended` (an Arc'd SessionTrace bracket on the MCP server — Clone-safe, fires on last drop) and `brief_served` on every brief — AI activity around the graph is retrievable, not just mutations of it. **Same day: write-time canon checks** — `Engine::canon_verdicts` (NLI on the write's near neighbors ≥ warn-similarity, cap 5): `supports` rides plain entailment ≥0.6 (canon already backs the text — the response tells the writer to LINK it instead of leaving reinforcement implicit), `contradicts` only inside the suspect band ≥0.7 (co-reference gate); rides `WriteOutcome::Created.canon` + `CheckedUpdate.canon`, surfaced by MCP with `canon_note` guidance, taught in all skill variants + skillgen. **Session-boundary graph validation** — `Engine::validate_graph()` (decay + conflict scan + drift count, journaled `graph_validated`) fired in a background thread by the MCP SessionTrace on connect AND disconnect (60s rate-limit, skipped on fake embeddings) — the graph is prepared for a session and cleaned after it without waiting for the six-hourly sweep; live-verified via a raw MCP initialize (journal: mcp_session_started → graph_validated). Audit-pane action chips size to their text (activity actions overflowed the fixed mutation-verb width). **Dogfood retune (2026-07-23):** the coherence sweep ("does the body support its title") was REMOVED after two live passes — 57 then 72 of ~150 healthy nodes at contradiction 1.00: even within one node (co-reference given) the MNLI-class runtime junk-poles to confident contradiction on this domain's dense technical text, in every input shape tried (400-char body prefix, first-sentence, neutral-dominance gating still left ~16 false positives). Third calibration lesson recorded; revisit with the domain-calibrated model (§7A phase 2). Stale-triage and canon checks survive — both ride the calibrated claim()-vs-claim() shape. **Version tracking (user feature, 2026-07-23):** optional per-graph (`GraphConfig.versioning.enabled` + per-type `TypeRoles.versioned` — Principle/Anchor off in the shipped set, Source/Rule off in the presets: values and code subjects transcend releases); the CURRENT WORKING VERSION lives in store meta (travels with migrate), free-form ≤32 chars; every new version-bound node is auto-stamped (`Node.version`, skip_serializing when absent — old dumps unchanged; explicit version wins for historical digestion); the brief leads with "Current working version: X" + set_version teaching; node lines carry the stamp; `set_version` MCP tool (set/unset, returns switch history — journaled `version_switched` under entity_id "version"), GET/PUT /version, redactor section + per-type checkbox + NodeDetail badge; update can correct a node's version. **Handoff notes (in place of a new "reactive" type — deliberately NOT an ontology change):** reserved tag `handoff` on an open worklist note = guaranteed FIRST brief section ("read first, resolve after acting", 400-char excerpts, never sampled away); resolution stops the surfacing, volatile decay burns forgotten leftovers; taught in all skill variants + skillgen. Timeline-cards fifth pane layout: **blocked on user design notes** (next session).

**Tracked risks:** cross-ontology home-graph promotion (role mapping), exports must embed their ontology definition, NLI claim templates per verb, skills teach-by-example templating. **Deferred cleanups (simplify pass 2026-07-23):** the write path runs four separate same-vector KNN queries (dupe check, warnings, suspects, canon) — unify into one candidate fetch when it next changes; `/refs/match` scans all_nodes per file-read — add a store-level refs lookup when graphs approach thousands; `validate_graph` holds the engine lock for the whole pass — split per phase if session-start stalls appear; skill generation prose lives CLI-side while `describe_ontology` lives in core — merge when the generated-skill feature grows.

---

## 7E. v0.7.1 — scope LOCKED 2026-07-24; RELEASED 2026-07-25 as the timeline release

*Status:* **shipped 2026-07-25** — the user cut the release once the UI design track (item 6) and `set_version` auto-enable (item 1) landed and dogfooded well; items 2–5 (downstream-use confirmation, session quarantine, simplify-pass cleanups, D-lite one-click daemon) **carry to the 0.7.2 cycle unchanged**. Release notes now live in `CHANGELOG.md` (one `## v<version>` section per release; draft-release.yml lifts it into the GitHub Release body — the convention from now on).

**Scope as locked (build order):**
1. **`set_version` auto-enables version tracking** — ***DONE 2026-07-24***: setting a version while tracking is off flips `versioning.enabled` on at the engine level (covers MCP and PUT /version uniformly — the stamp must never be silently ignored when a user asks a model to set the version); clearing never toggles; the MCP response says `versioning: was off — enabled by this call` when it flipped; tool description teaches it. Tests reworked + a dedicated toggle test (core 142).
2. **Downstream-use confirmation** [00agrffeot4m] — a `/refs/match` hit that fired in a session that ended well stamps `confirmed_at` on the matched node. Substrate exists (activity journal, MCP SessionTrace brackets, the refs hook); the work is session-outcome semantics. Completes §11's "successful downstream use" third confirmation signal — the last open piece of trust v2.
3. **Session quarantine** [009rn9x813tg] — quarantined-sessions table + retrieval filter + pane toggle; journal substrate shipped long ago; the designed pairing for digest batches (`session_id: digest-<date>`).
4. **Simplify-pass cleanups (§7D deferred):** unify the write path's four same-vector KNN queries (dupe / warnings / suspects / canon) into one candidate fetch; store-level lookup for `/refs/match` (drop the per-file-read all_nodes scan); per-phase locking in `validate_graph` only if session-start stalls actually appear.
5. **One-click daemon in both IDEs (D-lite):** the VSCode extension and JetBrains plugin get a single "Start Engram" action that launches `engram-alpha serve` (and surfaces its health), replacing the copy-a-command setup card for the daemon step. Skill + `.mcp.json` install deliberately stays the manual command for now — full bundling [009e36hgq6cu] and plugin-owned lifecycle [009k1619lkt6] remain the 0.7.2+ track.
6. **UI design track (user redirect 2026-07-24 — incremental redesign, mockup-driven; v1 SHIPPED same day, deployed for dogfood):** (a) **Feed view** — the timeline idea [00adhs2lr80m] realized as a second SCREEN, not a fifth canvas layout: a Graph/Feed switch in the topbar; a vertical centered card feed with depth peek (neighbors visibly behind/ahead, distance-scaled), two lenses — Timeline (chronological, version-marker separators) and Review (provisional/stale/drifted with inline Approve / Still true) — riding the existing canvas filters plus sort; edge chips traverse the graph with a session-scoped Back trail (cap 40, sessionStorage). (b) **Settings control kit** — native sliders/number inputs/checkboxes replaced by HueRail (spectrum track, hue-filled thumb), StepperInput (− value +), SegmentedControl, ToggleChip across the whole ConfigPanel; type cards carry their hue as a left spine. (c) **Topbar consistency** — brand glyph in a filled accent chip, ⌘K/Ctrl-K search shortcut + keycap hint, "+ New" as the one filled primary action, health strip in mono with dividers + a mean-trust "trust index" meter. **Dogfood rounds 2–5 (2026-07-25, all in the release):** stable-geometry card expansion (full markdown clipped to a fixed preview, centered card expands only after scroll settles — positions never drift), graph↔feed selection sync, zoom-gated glass, gradient accent spines + per-theme `--accent-wash`, dynamic-center drawers, solid search field in feed (Chromium backdrop-filter can't sample a composited scroller), health strip graph-only, opaque burger dropdown (backdrop-filter ancestor hijacks fixed-position drawers), 530/400px responsive breakpoints, docs (pane.md timeline section + README + fresh screenshots).

**Release-adjacent (user action, not code):** publish the Claude Code plugin to the directories [009ti2dyu9zn] — both forms only want the repo URL: Anthropic's official directory via https://clau.de/plugin-directory-submission (approval ⇒ installable as `engram@claude-plugins-official`, in-product discovery) and claudepluginhub.com/tools/submit-plugin (third-party index; the form skips the days-long GitHub Code Search auto-discovery). `/plugin marketplace add techtheist/engram` already works; `validate --strict` passes.

**Deferred out of 0.7.1:** model-download SHA-256 pinning (user: later; SECURITY.md keeps it tracked); timeline-cards fifth layout [00adhs2lr80m] (still blocked on user design notes — rides along only if they land); cortex continuation [009ygyac6mto] (edge-verb suggestions, offline split/merge, domain-calibrated NLI from the judged-suspects corpus, embed-model threshold recalibration); Phase 3 multi-harness; cross-ontology home promotion role mapping; anchor fuzzy re-match; app-level encryption (user: far down). 0.8.\* remains the SQLite-drop storage milestone.
...
- **Multi-project memory — §7C:** all five steps done (hub + federation on SQLite 2026-07-20, then the `Store` trait + TepinDB driver + `engram-alpha migrate` cutover the same day). tepindb 0.4.0 (2026-07-21) shipped the whole dossier — reset_embedder, native upsert, empty reads, open retry, unique indexes, `tepin migrate`, and in-driver serving (multi-process reads) — and 0.6.2 consumes it from crates.io: the daemon opens stores with ServeMode::Host, so `npx tepindb` inspects/queries a LIVE engram store through the sidecar. Remaining upstream ask: slim-npx `search` delegating to the host's embedder (phase 2, filed). The machine-level hub daemon LANDED 2026-07-21 (v0.6.2, uncommitted): one convergent core per machine on one port over every registered store (home-current when launched outside a project), per-project MCP at /projects/{id}/mcp so AI sessions bind to their repo, heartbeat-watchdog bridges that die with the core, and `engram-alpha stop` as the one-gesture cut. New graphs are born .tepin; the storage sunset is versioned (0.7.* migrate-by-default, 0.8.* drop SQLite — gated on tepin's format-migration story and the requirements dossier filed in the tepindb graph).

**v0.7.0 — the customization release (RELEASED 2026-07-23):** see §7D — all six workstreams shipped (tepin by default with silent auto-migrate, the GraphConfig substrate, the engine fully on live config, presets + the redactor UI, the teaching surface incl. ontology-generated skills, and the ws6 extras: file-read match hook, write-time canon checks, session-boundary validation, version tracking, handoff notes). Published as tag v0.7.0 (feat 5b451f7 + docs d785481). Timeline-cards layout did not make it — carried to §7E. **Current cycle: v0.7.1 — candidates in §7E.**

**Phase 2 — active features:** ~~mid-session conflict push~~ (shipped v0.6.3), ambient capture → librarian, cross-platform release pipeline.
