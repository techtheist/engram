...
## 7D. v0.7.0 — the customization release (scoped 2026-07-22)
...
**v0.7.0 — the customization release (workstreams 1–5 of 6 done as of 2026-07-23, all uncommitted draft):** see §7D — SHIPPED IN DRAFT: tepin by default with silent auto-migrate + export/import thin clients (ws1), the GraphConfig substrate (ws2), the engine fully on live config — string-backed ontology, role-flag behaviors, tunable policy, config-composed brief (ws3), presets + the ontology redactor UI with hue-derived colors + rename-as-bulk-retype (ws4), and the teaching surface incl. `describe_ontology` + ontology-generated skills installable from the pane (ws5, pulled forward from 0.7.1). REMAINING (ws6 extras + release): file-read match hook, timeline-cards layout, then commit + version bump + docs + the release itself.
