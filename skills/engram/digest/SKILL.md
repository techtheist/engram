...
**3. Read the reasoning surfaces (tier 2 — the real material).** List the tree first, then read individual FILES — a directory is never a readable target. README, design/plan docs and ADRs if present, top-level configs and manifests (lockfiles and CI reveal locked tool choices), the heads of the main modules. Use `git log --oneline -30` and targeted `git log --follow <file>` only to recover the *why* behind something you found in the tree. From this, extract: choices made for a reason (Decisions), stated or evident conventions (Principles), constraints and gotchas (Cautions), non-obvious realizations (Insights), unfinished threads (Intents), known open issues (Problems).

**4. Author nodes by the worked examples below.** Batch related notes into `add_notes` calls (≤100 items, per-item verdicts) — **items carry no links**; `link` is always a second pass over the ids the response returns. An unlinked pile is not a graph, so make that second pass immediately, batch by batch. (The worked examples show single `add_note` calls for clarity — the item shape inside `add_notes` is identical.)

**Date historical knowledge.** Digestion writes history, and history deserves its real place in time: when a source tells you when something happened — a commit date from `git log`, a dated ADR, a changelog entry — pass it as `created_at: "YYYY-MM-DD"` on the write, so the node lands on the timeline where the decision was made, not where the ingest ran. Omit `created_at` whenever the date is unknown or the knowledge is being stated fresh — never guess a date. (Old dates interact with durability honestly: a year-old episodic note starts stale, which is correct; historical decisions and conventions are `stable` and keep full trust regardless of age.)
