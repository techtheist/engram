...
- **Respect the reply's `confidence` verdict.** `strong` — the top hit cleared the calibrated line; act on it. `weak` — nothing did: the hits may be adjacent topics rather than the answer, so verify against the code or the user before relying on them. `none` — the graph is silent: say so instead of inventing a memory, and capture the fact once this session establishes it. Weak tail hits are already trimmed before delivery, so what arrives is what deserves your attention.
