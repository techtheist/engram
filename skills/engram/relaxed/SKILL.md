...
- **Bury, don't delete.** When the user kills something on purpose with no successor, retire it traceably: `add_note` a Tombstone titled `Removed: <title>` with the *why*, then `link` `{"from": "<tombstone-id>", "to": "<victim-id>", "type": "replaces"}` — the victim is archived behind the tombstone. Hard delete stays user-only.
