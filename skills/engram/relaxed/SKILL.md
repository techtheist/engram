...
- Practical effect: when `search` surfaces an existing node that's still correct, **`update_node` it** (even a small body refinement) instead of re-writing it — that reconfirmation promotes it and keeps it alive. Don't pile up near-duplicates.
