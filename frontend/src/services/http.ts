...
export const api = {
    ...
    /**
     * Live graph mutations, project-scoped. The daemon speaks SSE; the
     * contract the pane sees is "hand me frames until I unsubscribe", which
     * is why this returns a teardown instead of a URL — the static demo
     * (`frontend/demo/`) satisfies it with a local emitter and no server.
     */
    subscribe(handlers: StreamHandlers): () => void {
        const stream = new EventSource(`${API_BASE}${projectPrefix}/events`)
        stream.onopen = () => handlers.open()
        stream.onerror = () => handlers.error()
        stream.onmessage = (ev) => handlers.message(ev.data as string)
        return () => stream.close()
    },
}
